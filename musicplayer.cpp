...
{
    QSqlQuery query;
    if (!QFile(song->songUrl).exists()) {
        query.exec(QString("delete from songInformation where id = %1").arg(song->songID));
        query.exec(QString("select userName, likeMusicIndex from userInfo"));
        while (query.next()) {
            QString userName = query.value(0).toString();
            QStringList indexTemp = query.value(1).toString().split(",");
            qDebug() << indexTemp;
            indexTemp.removeOne(QString::number(song->songID));
            qDebug() << indexTemp;
            query.exec(QString("update userInfo set likeMusicIndex = \"%1\" where userName = \"%2\"")
                                .arg(indexTemp.join(","), userName));
        }
        return false;
    }
...
//    int timeLen = static_cast<int>(musicPlayer->duration() / 1000); //当前歌曲长度 单位为秒
    ...
    updateLrc(static_cast<double>((pos)));
}
...
//    int timeTotalLen = static_cast<int>(musicPlayer->duration() / 1000); //当前歌曲长度 单位为秒
    ...
    int timeTotalInts = timeTotalInt / 1000;
    int hh = timeTotalInts / 3600;
    int hm = (timeTotalInts % 3600) / 60;
    int hs = timeTotalInts % 60;
...
void MusicPlayer::loadLrcFile(QString filePath)
{
    ...
    while (!reader.atEnd() && re.isValid()) {
        ...
        int time = capturedText.at(1).toInt() * 60000 + capturedText.at(2).toInt() * 1000
                + capturedText.at(3).toInt();
        qDebug() << time;
